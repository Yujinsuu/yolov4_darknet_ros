...
def pub_detected(count):
    ...
    if (max_A >= thresh):
        ...
        print("A="+str(A_num+1)+"B_ypos : "+str(delivery_A))
    else:
    ...
    if (B_flag == 1):
        if all(b >= thresh for b in delivery_B):
            min_B = min(B_ypos)
            max_B = max(B_ypos)
            print("B="+str(A_num+1)+"B_ypos : "+str(delivery_A))
    ...
    pub_ID = rospy.Publisher('/delivery_ID', Int32MultiArray, queue_size=1)
...
if __name__ == '__main__':
    ...
    rospy.init_node('delivery_ID', anonymous=True)
